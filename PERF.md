...
[2\. Tests de charge \- k6](#tests-de-charge---k6)

[a. Configuration](#configuration)

[b. Résultats](#résultats)

[c. Seuils (thresholds)](<#seuils-(thresholds)>)

[d. Résumé global k6](#résumé-global-k6)

[3\. Tests de performance détaillés \- Newman](#tests-de-performance-détaillés---newman)

[a. Upload (POST /api/v1/files)](<#upload-(post-/api/v1/files)>)

[b. Download (POST /api/v1/download/:token)](<#download-(post-/api/v1/download/:token)>)

[c. Autres endpoints](#autres-endpoints)

[d. Résumé global Newman](#résumé-global-newman)

[4\. Logs structurés \- Analyse des métriques](#logs-structurés---analyse-des-métriques)

[a. Système de logs](#système-de-logs)

[b. Métriques clés observées dans les logs](#métriques-clés-observées-dans-les-logs)

[5\. Budget performance front (P4-FRONT)](<#5.-budget-performance-front-(p4-front)>)
...
3. ### **Seuils (thresholds)** {#seuils-(thresholds)}
...
4.  ### **Résumé global k6** {#résumé-global-k6}
...
3. ## **Tests de performance détaillés \- Newman** {#tests-de-performance-détaillés---newman}
   1. ### **Upload (POST /api/v1/files)** {#upload-(post-/api/v1/files)}
...
- Flux Multer \+ Prisma identique
  2. ### **Download (POST /api/v1/download/:token)** {#download-(post-/api/v1/download/:token)}
...
4. ### **Résumé global Newman** {#résumé-global-newman}
...
1. ### **Système de logs** {#système-de-logs}
...
2. ### **Métriques clés observées dans les logs** {#métriques-clés-observées-dans-les-logs}
